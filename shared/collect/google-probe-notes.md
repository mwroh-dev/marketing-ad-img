...
(Search terms / advertiser ids shown are **synthetic placeholders**; the navigation structure was live-verified.)
...
- 검증: 입력 "샘플광고주" → 첫 제안 hover-클릭 → `/advertiser/AR20000000000000000002?region=KR` 착지 → simgad 23개 캡처.

## 3. 광고주 페이지 = 직접 URL 작동
- `https://adstransparency.google.com/advertiser/AR<id>?region=KR` 직접 navigate 시 그 광고주 광고 렌더(샘플광고주 AR1782… → 13개). AR id를 알면 재진입 가능(provenance/재실행).
...
## 5. region 의미 + 차단
- `region=KR` = **광고 게재 지역** 필터지 광고주 본사 아님(샘플광고주 검색→미국 광고주들). KR 경쟁사는 한글 브랜드명으로 검색해야 KR 광고주가 뜸.
