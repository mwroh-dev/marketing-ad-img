...
2. The agent writes a single artifact `→ .generate-ads-img/runs/{run_id}/request-evaluation.json` (schema `${CLAUDE_PLUGIN_ROOT}/schemas/evaluation/request-evaluation.schema.json`) carrying: `detected_mode` (`initial-setup | data-collection | image-generation | performance-learning | unknown`), `mode_confidence`, `required_slots[]`, `slot_states[]`, `blockers[]` (each `slot` + `type` `hard_block|soft_block` + `priority` + `reason`), `ready`, `next_interview_target` (null or `{slot, rationale}`), `risk_flags[]`.
