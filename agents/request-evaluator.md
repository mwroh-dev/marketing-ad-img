...
- **creative-change-analysis**: brand_id, product_id, persona_id, snapshot_selection, analysis_store. Hard-block when persona_id is missing, the persona has no collection snapshot, `analysis_validate_store` fails, or an edge analysis is requested with fewer than 2 dated snapshots.
