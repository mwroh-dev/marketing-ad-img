...
- `validate-recipe` → require ≥1 collection run for the persona (0 → route to data-collection). Run `node ${CLAUDE_PLUGIN_ROOT}/shared/collect/validate-recipe.mjs <persona_id>` with `run_in_background: true`; it prints `SELECT_URL …` — relay it. **Read-only: no POST, no write/move.** Tell the user red-badged cards may be weak analyses and to copy an ad's 📋 id → ask to re-analyze here. On a re-analysis request, locate the ad by the pasted id and re-run `modes/analysis.md` for that one image. Never let the user inline-edit a schema (overwrites grounds_in/confidence discipline).
