...
| analysis (typed · text⊥layout⊥visual⊥intent) | perception→ad-type-classify(route)→(copy⊥layout⊥visual)→intent→pattern + keyword | agents `perception-extractor`,`ad-type-classifier`,`copy-analyst`,`layout-analyst`,`visual-analyst`,`intent-analyst`,`ad-analyst`,`pattern-synthesizer` + `keyword-rank`,`ad-pattern-rank`,`slice-stitch`,`bbox-bind`,`ad-type-registry` · taxonomy `knowledge/reference/ad-taxonomy.md` |
