...
| competitive-report | per-persona competitive intelligence from collected creatives across dated snapshots: longevity (run-duration = longevity proxy) + variation/cadence + new/disappeared + appeals → consumer HTML report | runbook `modes/competitive-report.md` · tools `report_aggregate_competitive_trend`,`report_render_competitive` (deterministic) + agent `competitive-analyst` |
| creative-change-analysis | compare persisted creative snapshots: create/delete/persisted/update + classified distribution shifts + claim-kind-separated interpretation/context hypotheses | runbook `modes/creative-change-analysis.md` · tools `creative_change_build_snapshot`,`creative_change_compare_snapshots`,`creative_change_detect_candidates` (deterministic) + agents `temporal-change-analyst`,`market-context-researcher` (optional) + tool `creative_change_render_report` |
| validate-recipe | on-demand READ-ONLY QA viewer of the analysis we extracted: per ad (grouped by collection date) image + info + recipe (type/copy/register/benefit×funnel/cognition) shown faithfully. NO system quality verdict — the human compares each ad to its recipe and judges (the agent must not pre-grade; it can be confidently wrong). Correction is a terminal conversation (copy the ad's id → "이 광고 재분석해줘"), never an inline edit | runbook `modes/validate-recipe.md` · script `validate-recipe` (deterministic template-fill, no agent, no write) |
| generation | opportunity(ring3)→brief→copy→adapter→critic→finalizer | runbook `modes/image-generation.md` · agents `creative-opportunity-mapper`,`creative-brief-analyst`,`copy-layout-planner`,`image-prompt-adapter`,`critic-verifier` + tool `generation_finalize_candidates` |
...
- `shared/` — execution logic (shared): `shared/tools/` (ToolSpec catalog + MCP adapter: tool id → MCP name → current implementation), `shared/collect/` (collection · deterministic ranking/gating · slicer · acquire-port), `shared/validators/validate-*.ts`, `shared/harness/` (run-* · finalizer · cutout), `shared/_lib.ts`.
