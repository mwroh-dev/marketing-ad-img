...
**Do NOT pre-read `knowledge/reference/subagent-projection.md`, all of `knowledge/`, or the design docs.** Loading the whole repo up front wastes context and tokens and is not how the harness works. The orchestrator *routes*; each mode/agent loads only its own contract when it becomes active (progressive disclosure).
