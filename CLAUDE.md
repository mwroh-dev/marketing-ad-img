...
| analysis (text⊥layout⊥visual⊥intent) | perception→(copy⊥layout⊥visual)→intent→pattern + keyword | agents `perception-extractor`,`copy-analyst`,`layout-analyst`,`visual-analyst`,`intent-analyst`,`ad-analyst`,`pattern-synthesizer` + `keyword-rank`,`ad-pattern-rank`,`slice-stitch`,`bbox-bind` |
